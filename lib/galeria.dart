...
class GalleryImage {
  ...
  //Convert JSON object to GalleryImage object
...
Future<List<GalleryImage>> fetchImages() async {
  final response = await http.get("http://10.0.2.2:8000/api/gallery/");//TODO: Replace with original API endpoint

  if (response.statusCode == 200) {
    var imageObjJson = json.decode(utf8.decode(response.bodyBytes)) as List;
    List<GalleryImage> imageList = imageObjJson
...
class _GaleriaPage extends State<GaleriaPage> {
  final Future<List<GalleryImage>> images = fetchImages(); //list of images
  double width = 0.29;
              ...
              Flexible(
                  child: FutureBuilder(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.hasData) {
                          List<GalleryImage> obrazy = snapshot.data; //downloaded images
                          //if images is loaded
                          return Padding(
                                  ...
                                  Row(
                                    ...
                                    children: <Widget>[
                                      Column(
                                          ...
                                          children: obrazy.map((item) { //from images create widgets
                          ...
                          //if images is loading
                          return Center(child: CircularProgressIndicator()); //return loading progress indicator
                        }
                      }))
            ],
          )
  ...
  Widget build(BuildContext context) {
    return Padding(
        ...
        child: GestureDetector(
          child: Hero(
            tag: url.split('/').removeLast(), //tag must be unique
            child: Container(
              width: szerokosc,
    ...
    return Scaffold(
      ...
      body: Column(
        children: <Widget>[
          ...
          Center(
            child: Hero(
              tag: this.url.split('/').removeLast(), //tag must be unique
              child: Container(
